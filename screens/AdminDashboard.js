...
const AdminDashboard = ({ navigation }) => {
    ...
    useEffect(() => {
        const fetchPendingAlerts = async () => {
            try {
                ...
                const response = await axios.get('https://3ad1-92-236-121-121.ngrok-free.app/api/alerts/pending', {
                    headers: {
    ...
    const handleAlertAction = async (alertId, action) => {
        try {
            const token = await AsyncStorage.getItem('token');
            await axios.post(`https://3ad1-92-236-121-121.ngrok-free.app/api/alerts/${alertId}/${action}`, {}, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
